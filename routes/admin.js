...
router.get('/orders/:orderId', (req, res) => {
  ...
  const item = db.prepare('SELECT * FROM order_items WHERE order_id = ?').get(req.params.orderId);
...
router.patch('/orders/:orderId/status', (req, res) => {
  ...
  const order = db.prepare('SELECT id FROM orders WHERE order_id = ?').get(orderId);
  if (!order) return res.status(404).json({ error: 'Order not found' });
