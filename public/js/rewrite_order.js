...
const file = path.join('c:', 'Users', 'Wali', 'Downloads', 'silai stuff', 'silaibox', 'public', 'js', 'order.js');
...
if (countrySelect) {
  countrySelect.addEventListener('change', () => {
    // If user changes checkout country, prompt them to change global currency if wanted
...
function populateReview() {
  const reviewFabric = document.getElementById('review-fabric');
  if (reviewFabric) {
    let t = document.getElementById('fabric-type')?.value || 'Not specified';
    reviewFabric.textContent = t;
  }
}
  ...
  orderForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (resp.ok && data.success) {
        ...
        document.getElementById('success-btn').onclick = () => { window.location.href = trackUrl; };
      } else {
...
fs.writeFileSync(file, orderScript, 'utf8');
console.log('Order.js completely rewritten smoothly and functional!');
